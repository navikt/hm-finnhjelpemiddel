...
export default function EditAlternativeProductsPage({ userToken }: { userToken: string }) {
  ...
  return (
    <VStack gap={'4'} className={`${styles.container} main-wrapper--large`}>
      ...
      {searchParams.has('hms') && <AlternativeGroupList hmsNumber={searchParams.get('hms')!} userToken={userToken} />}
    </VStack>
  )
}

const AlternativeGroupList = ({ hmsNumber, userToken }: { hmsNumber: string; userToken: string }) => {
  const {
    data: alternativesResponse,
    isLoading: isLoadingAlternatives,
    mutate: mutateAlternatives,
    error: errorAlternatives,
  } = useSWRImmutable(`asdasd-${hmsNumber}`, () => newGetAlternatives(hmsNumber, userToken))
