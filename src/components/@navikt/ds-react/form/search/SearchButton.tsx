...
import { composeEventHandlers } from '@navikt/ds-react/src/util/composeEventHandlers'
...
const SearchButton: SearchButtonType = forwardRef(({ className, children, disabled, onClick, ...rest }, ref) => {
  ...
  return (
    <Button
      ...
      icon={<MagnifyingGlassIcon {...(children ? { 'aria-hidden': true } : { title: 'Søk' })} />}
    >
      {children}
    </Button>
  )
